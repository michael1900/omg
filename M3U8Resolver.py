...
import traceback # Importa traceback per un logging degli errori più dettagliato
...
def resolve_m3u8_link(url, headers=None):
    ...
    Restituisce una stringa formattata come voce M3U8 con EXTVLCOPT o None in caso di errore.
    """
    if not url:
    ...
    try:
        ...
        with requests.Session() as session:
            ...
            print(f"Passo 1: Richiesta a {url} con headers {current_headers}")
            ...
            print(f"Passo 1 completato. URL finale dopo redirect (prima di cercare iframe): {final_url_after_redirects}")
            ...
            try:
                ...
                # NOTA: Qui usiamo url2 per la richiesta all'iframe
                referer_raw = urlparse(url2).scheme + "://" + urlparse(url2).netloc
                current_headers['Referer'] = url2
                current_headers['Origin'] = referer_raw
                print(f"Passo 3 (Iframe): Richiesta a {url2} con headers aggiornati {current_headers}")
                ...
                # !!! MODIFICA QUI: Stampa i parametri estratti per chiarezza !!!
                print(f"  - channel_key: {channel_key}")
                print(f"  - auth_ts: {auth_ts}")
                print(f"  - auth_rnd: {auth_rnd}")
                print(f"  - auth_sig (encoded): {auth_sig}")
                print(f"  - auth_host: {auth_host}")
                print(f"  - server_lookup: {server_lookup}")


                # Quinto passo (Iframe): Richiesta di autenticazione (questa richiesta potrebbe non restituire direttamente la server_key)
                auth_url = f'{auth_host}{channel_key}&ts={auth_ts}&rnd={auth_rnd}&sig={auth_sig}'
                # Mantieni gli stessi header dell'iframe request
                print(f"Passo 5 (Iframe): Richiesta di autenticazione a {auth_url} con headers {current_headers}")
                auth_response = session.get(auth_url, headers=current_headers, timeout=10)
                auth_response.raise_for_status()
                # Non tentiamo di estrarre server_key qui, la faremo dalla prossima richiesta
                print("Passo 5 (Iframe) completato.")

                # Sesto passo (Iframe): Richiesta di server lookup per ottenere la server_key
                # NOTA: Qui usiamo url2 per costruire la server lookup URL come nella logica originale
                server_lookup_url = f"https://{urlparse(url2).netloc}{server_lookup}{channel_key}"
                print(f"Passo 6 (Iframe): Richiesta server lookup a {server_lookup_url} con headers {current_headers}")
                ...
                # !!! MODIFICA QUI: Costruisci la stringa M3U8 con EXTVLCOPT headers !!!
                # Usa i valori raw per gli header nelle direttive EXTVLCOPT
                m3u8_entry = (
                    f'#EXTINF:-1 tvg-id="Resolved Stream" tvg-name="Resolved Stream" tvg-logo="" group-title="Resolved", Resolved Stream\n'
                    f'#EXTVLCOPT:http-referrer={referer_raw}\n'
                    f'#EXTVLCOPT:http-user-agent={current_headers.get("User-Agent", "")}\n'
                    f'#EXTVLCOPT:http-origin={referer_raw}\n'
                    f'{final_stream_url}'
                )

                print(f"M3U8 Entry generato:\n{m3u8_entry}")
                response.close() # Chiudi la connessione
                return m3u8_entry # Restituisce la stringa formattata
                ...
                if initial_response_text and initial_response_text.strip().startswith('#EXTM3U'):
                    ...
                    # Per il fallback, restituisci solo l'URL diretto, senza EXTVLCOPT
                    response.close() # Chiudi la connessione
                    return final_url_after_redirects
                else:
...
if __name__ == "__main__":
    # Esempio di utilizzo:
    # Questo esempio richiede di avere M3U8Parser.py nella stessa directory
    try:
        from M3U8Parser import parse_m3u8_file
    except ImportError:
        print("Errore: M3U8Parser.py non trovato. Assicurati che sia nella stessa directory.")
        sys.exit(1)
    ...
    if channel_list:
        # Prova a risolvere il link del primo canale
        first_channel = channel_list[0]
        print(f"\nTentativo di risolvere il link per il canale: {first_channel['name']}")
        # Passa gli header estratti dal parser, se disponibili
        resolved_output = resolve_m3u8_link(first_channel['url'], first_channel.get('headers'))

        if resolved_output:
            print(f"\nOutput risolto per '{first_channel['name']}':")
            print(resolved_output) # Stampa l'intera stringa M3U8 entry
            # Qui potresti aggiungere la logica per riprodurre l'output risolto
        else:
            print(f"\nImpossibile risolvere il link per '{first_channel['name']}'.")
    else:
        print("Nessun canale disponibile per la risoluzione.")